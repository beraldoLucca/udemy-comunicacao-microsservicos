...
import { SUCCESS, INTERNAL_SERVER_ERROR } from "../../../config/constants/httpStatus.js";
...
class OrderService {
    async createOrder(req) {
        try {
            ...
            console.info(`Request to POST new order with data ${JSON.stringify(
                orderData
            ...
            console.info(`Response to POST new order with data ${JSON.stringify(
                response
            )} | [transactionID: ${transactionid} | serviceID: ${serviceid}]`)
            return response
        } catch (err) {
    ...
    async findAll() {
        try {
            ...
            if(!orders){
                throw new OrderException(BAD_REQUEST, "TNo orders were found.")
            }
            let response = {
                status: SUCCESS,
                orders,
    ...
    async findByProductId(req) {
        try {
            ...
            const {transactionid, serviceid} = req.headers;
            console.info(`Request to GET orders by productID: ${productId}
             | [transactionID: ${transactionid} | serviceID: ${serviceid}]`)
